...
//            $chatOriginal=$a['chats'];
    ...
    {
		$qzapVis = mysql_query("SELECT sum(recordsvk) as ttt FROM `masters` mx, `master_procedure_day` m, `procedures` p, m_city ci where ci.name='".$city."' and mx.id_m_city=ci.id and mx.`id`=m.`id_master` and m.`id_procedure`=p.id and `m`.`dt` BETWEEN '".$dateNachalaSQLZap."' and '".$dateKoncaSQLZap."' and count_in_scores>0");
