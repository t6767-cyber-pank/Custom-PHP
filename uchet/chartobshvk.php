...
include("class/pDrawAllChats.class.php");
...
$qrqcity = mysql_query("SELECT DISTINCT c.name as cname FROM `m_city` c, `masters` m WHERE m.`id_m_city`=c.id ORDER BY m.`sort` ASC");
    //"SELECT c.name as cname FROM `m_city` c, `masters` m, `master_week` mw WHERE c.id=m.id_m_city and m.shown>0 and m.id=mw.id_master GROUP by c.name");
$xz=0;
while ($cityx = mysql_fetch_array($qrqcity)) {
    $xz++;
    $city = $cityx['cname'];
    $sborvnedelyu = 0;
    ...
    $dateObrabotka=strtotime($dateNachala);
    /* ??? */
    $r = mysql_query("SELECT * FROM `m_city_day_vk` d, m_city m where d.`id_m_city`=m.id and m.name='" . $city . "' and dt='" . $dateNachalaVoskreseniya . "' order by `id_m_city`, `dt`");
    while ($a = mysql_fetch_array($r))  // Переборка данных с базы в данные приемлимые обработке
    {
        $chatOriginal = $a['chatsvk'];
    }
    /* ??? */
    while ($dateObrabotka != strtotime($dateKonca) + 86400) {
        if (date("D", $dateObrabotka) == "Mon") {
            ...
            $qzap = mysql_query("SELECT * FROM `m_city_day_vk` d, m_city m where d.`id_m_city`=m.id and m.name='" . $city . "' and dt BETWEEN '" . $dateNachalaSQLZap . "' AND '" . $dateKoncaSQLZap . "' order by `dt`");
//        echo $dateNachalaSQLZap."-".$dateKoncaSQLZap."   ";
        ...
        if (date("D", $dateObrabotka) == "Sun") {
            $qzapoutcome = mysql_query("SELECT * FROM `m_city` c, `masters` m, `master_week` mw WHERE c.id=m.id_m_city and m.id=mw.id_master and c.name='" . $city . "' and mw.dt BETWEEN '" . $dateNachalaSQLZap . "' AND '" . $dateKoncaSQLZap . "' ORDER BY mw.dt DESC");
            while ($aoutcome = mysql_fetch_array($qzapoutcome)) {
                $oc = (int)$aoutcome['outcomevk'];
            ...
            array_push($outcome, round($sborvnedelyuoutcome /1000));  //  Кидаем в массив чата данные с базы
            ...
            //echo $city."".$dateKoncaSQLZap." --- ".$sborvnedelyu."<br>";
...
$countarray=count($chats);
$predel=$countarray/$xz;
...
$i=0;
foreach ($chats as $ch) {
    $chatsx[$i]=(int)$chatsx[$i]+(int)$ch;
    $i++;
    if ($i==$predel) { $i=0;}
    }

$i=0;
foreach ($outcome as $ou) {
    $outcomex[$i]=(int)$outcomex[$i]+(int)$ou;
    $i++;
    if ($i==$predel) { $i=0;}
}
...
/*
if ($rash>0) {
    $myPicture->drawText(5, 5, "Итого: " . $summaChat . " чатов " . $summaoutcome . " тг", $TextSettings);
} else
{   $myPicture->drawText(5,5,"Итого: ".$summaChat." чатов / ".$summaoutcome." тг",$TextSettings); } */
...
if ($rash>0) {
    ...
    $myPicture->drawText(860,95,"Цена за\n  1 чат:",$TextSettings);
    $myPicture->setFontProperties(array("FontName"=>"fonts/arialbd.ttf","FontSize"=>32));
    $myPicture->drawText(870,175, round($lid),$TextSettings);
}
*/
...
//if ($rash>0) { $razmerholstaArea=$razmerholsta-150; }
...
, "Mode"=>SCALE_MODE_MANUAL //SCALE_MODE_FLOATING //SCALE_MODE_MANUAL //SCALE_MODE_START0
...
$minus=104;
if ($razmerPodgon==4){ $minus=360; }
if ($razmerPodgon==5){ $minus=285; }
if ($razmerPodgon==6){ $minus=240; }
if ($razmerPodgon==7){ $minus=210; }
if ($razmerPodgon==8){ $minus=185; }
if ($razmerPodgon==9){ $minus=170; }
if ($razmerPodgon==10){ $minus=155; }
if ($razmerPodgon==11){ $minus=145; }
if ($razmerPodgon==12){ $minus=135; }
if ($razmerPodgon==13){ $minus=130; }
if ($razmerPodgon==14){ $minus=125; }
if ($razmerPodgon==15){ $minus=120; }
if ($razmerPodgon==16){ $minus=115; }
if ($razmerPodgon==17){ $minus=110; }
