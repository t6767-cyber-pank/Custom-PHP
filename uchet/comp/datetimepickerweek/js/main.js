/**Стрелочки**/
function lf(napr) {
    if(napr==0) { d.setDate(d.getDate() - d.getDay() - 2); }
    else
    ...
    $.ajax({
        ...
        success:function(html){
            document.getElementById('search-to-daten').value=parserDate(getMonday(d))+" - "+parserDate(getSunday(d));
            $('#ajax').html(html);
        },
        error:function(html){
...
jQuery(document).ready(function () {
    ...
    jQuery('#search-to-daten').datetimepicker({
        ...
        onChangeDateTime: function () {document.getElementById('search-to-daten').value=parserDate(getMonday(d))+" - "+parserDate(getSunday(d));},
