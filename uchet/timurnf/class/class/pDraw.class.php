   ...
   function drawScale($Format="")
    {
     ...
     $ManualScale	= isset($Format["ManualScale"]) ? $Format["ManualScale"] : array("0"=>array("Min"=>0,"Max"=>$this->scale));
           ...
           foreach($PosArray as $Key => $Y)
            {
              ...
              if ( $Serie["Data"][$Key] > 0 ) { $Align = TEXT_ALIGN_BOTTOMMIDDLE; $Offset = $DisplayOffset; } else { $Align = TEXT_ALIGN_TOPMIDDLE; $Offset = -$DisplayOffset; }
               $bx=$Serie["Data"][$Key];
               if ($Serie["Data"][$Key]>$this->scale) { $Serie["Data"][$Key]=$this->scale; $bx="(".$bx.")"; }
               $k=$Y;
               if ($Y<30) { $Y=30; $k=70; }
