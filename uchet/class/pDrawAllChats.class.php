   ...
   function drawScale($Format="")
    {
     ...
     $ManualScale	= isset($Format["ManualScale"]) ? $Format["ManualScale"] : array("0"=>array("Min"=>0,"Max"=>1000));
     ...
     foreach($Data["Series"] as $SerieName => $Serie)
      {
       if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
        {
            if ($colorCounter==0) {$R = 166; $G = 228; $B = 1; $Alpha = 100; $Ticks = $Serie["Ticks"]; $ForceTransparency=25; }
            else {$R = 255; $G = 0; $B = 0; $Alpha = 50; $Ticks = $Serie["Ticks"]; $ForceTransparency=30; }
         $colorCounter++;
         if ( $DisplayColor == DISPLAY_AUTO ) { $DisplayR = $R; $DisplayG = $G; $DisplayB = $B; }
           ...
           foreach($PosArray as $Key => $Y)
            {
              if ( $DisplayValues && $Serie["Data"][$Key] != VOID )
              {
               if ( $Serie["Data"][$Key] > 0 ) { $Align = TEXT_ALIGN_BOTTOMMIDDLE; $Offset = $DisplayOffset; } else { $Align = TEXT_ALIGN_TOPMIDDLE; $Offset = -$DisplayOffset; }
               $bx=$Serie["Data"][$Key];
               if ($Serie["Data"][$Key]>1000) { $Serie["Data"][$Key]=1000; $bx="(".$bx.")"; }
               $k=$Y;
               if ($Y<30) { $Y=30; $k=70; }
                  if ($userTimurCounter>0){
                      ...
                      if ($bx==0) $k=22;
                      $bx=$bx." тыс";
                      $this->drawText($X,$k-10,$this->scaleFormat($bx,$Mode,$Format,$Unit),array("R"=>113,"G"=>0,"B"=>3,"Align"=>$Align));
                      $this->drawText($X,$k+5,$this->scaleFormat($lid." тг",$Mode,$Format,$Unit),array("R"=>0,"G"=>0,"B"=>0,"Align"=>$Align));
                  }
                  else {$this->drawText($X,$k-10,$this->scaleFormat($bx,$Mode,$Format,$Unit),array("R"=>34,"G"=>113,"B"=>0,"Align"=>$Align, "FontSize"=>10, "FontName"=>"fonts/arialbd.ttf")); }
              }
