...
include($_SERVER['DOCUMENT_ROOT']."/mysql_connect.php"); // Подключаем модуль подключения к базе данных
   ...
   function drawScale($Format="")
    {
     ...
     $ManualScale	= isset($Format["ManualScale"]) ? $Format["ManualScale"] : array("0"=>array("Min"=>0,"Max"=>100));
             ...
             for($i=0;$i<=$Parameters["Rows"];$i++)
              {
               ...
               if ( $this->isValidLabel($Value,$LastValue,$LabelingMethod,$ID,$LabelSkip) && !$RemoveXAxis)
                {
                    $zap = mysql_query("SELECT * FROM pr_tovar where pokaz>0 and name='$Value' order by name");
                    $azap = mysql_fetch_array($zap);

                    $xbb=10;
                    $xr=0;
                    $xg=0;
                    $xb=0;
                    if ($azap['active']==0) { $xbb=10; $xr=192; $xg=192; $xb=192; }

                 $Bounds  = $this->drawText($XPos-$OuterTickWidth+$XLabelOffset,$YPos, $Value,array("Angle"=>$LabelRotation,"Align"=>$LabelAlign, "FontName"=>"fonts/arial.ttf","FontSize"=>$xbb, "R"=>$xr,"G"=>$xg,"B"=>$xb));
             ...
             for($i=0;$i<=$Parameters["Rows"];$i++)
              {
               ...
               $Bounds    = $this->drawText($XPos,$YPos-$OuterTickWidth-2,$Value,array("Align"=>TEXT_ALIGN_BOTTOMMIDDLE,  "FontName"=>"fonts/arialbd.ttf","FontSize"=>6));
     ...
     function drawScale2($Format="")
     {
         ...
         $MinDivHeight	= isset($Format["MinDivHeight"]) ? $Format["MinDivHeight"] : 20;
         $Factors		= isset($Format["Factors"]) ? $Format["Factors"] : array(1,2,5);
         $ManualScale	= isset($Format["ManualScale"]) ? $Format["ManualScale"] : array("0"=>array("Min"=>0,"Max"=>100000));
         $XMargin		= isset($Format["XMargin"]) ? $Format["XMargin"] : AUTO;
         $YMargin		= isset($Format["YMargin"]) ? $Format["YMargin"] : 0;
         $ScaleSpacing	= isset($Format["ScaleSpacing"]) ? $Format["ScaleSpacing"] : 15;
         $InnerTickWidth	= isset($Format["InnerTickWidth"]) ? $Format["InnerTickWidth"] : 2;
   ...
   function drawAreaChart($Format=NULL)
    {
     ...
     foreach($Data["Series"] as $SerieName => $Serie)
      {
       if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
        {
         if ($colorCounter==0) {$R = 166; $G = 228; $B = 1; $Alpha = 100; $Ticks = $Serie["Ticks"];}
         else {$R = 0; $G = 0; $B = 0; $Alpha = 0; $Ticks = $Serie["Ticks"];}
         $colorCounter++;
         if ( $DisplayColor == DISPLAY_AUTO ) { $DisplayR = $R; $DisplayG = $G; $DisplayB = $B; }
         ...
         if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
          {
           ...
           if ( !is_array($PosArray) ) { $Value = $PosArray; $PosArray = ""; $PosArray[0] = $Value; }
             /* echo "<pre>";
              ...
              echo "</pre>";
            */
            //  echo $Data["Series"]["TTT"]["Data"][1];
            //  echo $Data["Series"]["TT2"]["Data"][1];
           foreach($PosArray as $Key => $Y)
            {
              if ( $DisplayValues && $Serie["Data"][$Key] != VOID )
              {
               if ( $Serie["Data"][$Key] > 0 ) { $Align = TEXT_ALIGN_BOTTOMMIDDLE; $Offset = $DisplayOffset; } else { $Align = TEXT_ALIGN_TOPMIDDLE; $Offset = -$DisplayOffset; }
               $bx=$Serie["Data"][$Key];
               if ($Serie["Data"][$Key]>70) { $Serie["Data"][$Key]=70; $bx="(".$bx.")"; }
               $k=$Y;
               if ($Y<30) { $Y=30; $k=80; }
                  if ($userTimurCounter>0){
                      $os1=$Data["Series"]["TTT"]["Data"][$Key];
                      $os2=$Data["Series"]["TT2"]["Data"][$Key];
                      if ($os1>0){
                      $lid=Round($os2*1000/$os1);
                      }else { $lid=0; }
                      $bx=" ".$bx."т\n(".$lid.")";
                      $k=60;
                  }
                  $this->drawText($X,$k-10,$this->scaleFormat($bx,$Mode,$Format,$Unit),array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>$Align));
              }
     ...
     /* Draw an area chart */
     function drawAreaChart2($Format=NULL)
     {
         $DisplayValues	= isset($Format["DisplayValues"]) ? $Format["DisplayValues"] : FALSE;
         $DisplayOffset	= isset($Format["DisplayOffset"]) ? $Format["DisplayOffset"] : 2;
         $DisplayColor	= isset($Format["DisplayColor"]) ? $Format["DisplayColor"] : DISPLAY_MANUAL;
                 ...
                 if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
                 {
                     ...
                     foreach($PosArray as $Key => $Y)
                     {
                         if ( $DisplayValues && $Serie["Data"][$Key] != VOID )
                         {
                             if ( $Serie["Data"][$Key] > 0 ) { $Align = TEXT_ALIGN_BOTTOMMIDDLE; $Offset = $DisplayOffset; } else { $Align = TEXT_ALIGN_TOPMIDDLE; $Offset = -$DisplayOffset; }
                             $bx=$Serie["Data"][$Key];
                             if ($Serie["Data"][$Key]>100) { $Serie["Data"][$Key]=100; $bx="(".$bx.")"; }
                             $k=$Y;
                             if ($Y<30) { $Y=30; $k=45; }

                             $this->drawText($X,$k-10,$this->scaleFormat("(".$bx." тыс.)",$Mode,$Format,$Unit),array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>$Align));
                         }
       ...
       if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
        {
         ...
         $Color1 = array("R"=>190,"G"=>190,"B"=>190,"Alpha"=>$Alpha,"BorderR"=>$BorderR,"BorderG"=>$BorderG,"BorderB"=>$BorderB);
         $Color2 = array("R"=>$R,"G"=>$G,"B"=>$B,"Alpha"=>$Alpha,"BorderR"=>$BorderR,"BorderG"=>$BorderG,"BorderB"=>$BorderB);
               ...
               if ( $DisplayValues && $Serie["Data"][$Key] != VOID )
                {
                 ...
                 if ( $DisplayPos == LABEL_POS_INSIDE && abs($TxtHeight) < abs($BarHeight) )
                 ...
                 else
                  {
                   if ( $Serie["Data"][$Key] >= 0 ) { $Align = TEXT_ALIGN_BOTTOMMIDDLE; $Offset = $DisplayOffset; } else { $Align = TEXT_ALIGN_TOPMIDDLE; $Offset = -$DisplayOffset; }
                      $bx=$Serie["Data"][$Key];
                  if ($Serie["Data"][$Key]>20) { $Serie["Data"][$Key]=20; $bx="Макс (".$bx.")"; }
                  if ($Y2<30) $Y2=50;
                   $this->drawText($X+$XOffset+$XSize/2,$Y2-$Offset,$this->scaleFormat($bx,$Mode,$Format,$Unit),array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>$Align,"FontSize"=>$DisplaySize));
                  }
               ...
               if ( $Serie["Data"][$Key] == 0 )
               ...
               else
                {
                 ...
                 if ( $Rounded )
                  ...
                  {
                      $zap = mysql_query("SELECT * FROM pr_tovar where pokaz>0 and name='".$Data["Series"]["Months"]["Data"][$Key]."' order by name");
                      $azap = mysql_fetch_array($zap);
                      if ($azap['active']==0) { $Color=$Color1; } else { $Color=$Color2; }
