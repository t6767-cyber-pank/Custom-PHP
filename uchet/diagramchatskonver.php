...
<form action="/diagramchatskonver.php" method="GET">
  Дата начала:
...
$r = mysql_query("SELECT DISTINCT c.name, c.id as cityid FROM users u, `m_city` c, `masters` m WHERE u.id=m.id_master and m.`id_m_city`=c.id ".$usermanagertext."ORDER BY m.`sort` ASC"); //SELECT * FROM m_city order by name");
$counter=1;
