...
<form action="/diagramchats2.php" method="GET">	
  Дата начала:
...
while ($a = mysql_fetch_array($r)){
    ...
    if ($qsrav["chatcount"]>10) { //echo $qsrav["chatcount"];
        $qcitymasters=mysql_query("SELECT u.name FROM users u, `m_city`c, `masters` m where u.id=m.`id_master` and c.id=m.`id_m_city` and c.id=".$a['cityid']." and m.shown>0 ORDER BY `u`.`name` ASC");
        ?>
        ...
        <div id="content" class="content" align="center">
            <br/><h5><?php echo $a['name']." (";
                ...
                echo ")"; ?></h5>
            <p><img
                            src="chartManager.php?dateN=<?php echo $date1 ?>&dateK=<?php echo $date2 ?>&city=<?php echo $a['name']; ?>"
