...
if(isset($_POST['obrik']))
{
$sum=0;
for ($i=1; $i<$_POST['counter']+1; $i++)
{
	$suxummm=$_POST['kolvo'.$i]*$_POST['cenazaed'.$i];
	mysql_query('update spisok set prim="'.$_POST['prim'.$i].'", status='.$_POST[$i].', kolvo='.$_POST['kolvo'.$i].', summa='.$suxummm.' where id_spis='.$_POST['x'.$i]);
	$sum+=$_POST[$i];
}
if ($_POST['counter']==$sum)
{
	mysql_query('update zayavka set sostoyanieid=5, na_obrabotkeid=6, datesleg="'.$_POST["datesleg"].'Дата утверждения тех. дир.: '.date("d.m.y").'<br>'.'" where nomer='.$_POST["nomer"]);
}
else
{
	mysql_query('update zayavka set sostoyanieid=9, na_obrabotkeid=12 where nomer='.$_POST["nomer"]);
}
}
$qr_result = mysql_query("select * from zayavka z, sostoyanie s, users u where z.sostoyanieid=s.id_sost and z.na_obrabotkeid=u.id_polz and sostoyanieid=10 order by z.nomer asc");
...
echo '<table border="1">';
  ...
  while($data = @mysql_fetch_array($qr_result)){ 
    echo '<tr>';
	echo '<td><form method="POST" action="odobreniezaytehdirector.php">';
