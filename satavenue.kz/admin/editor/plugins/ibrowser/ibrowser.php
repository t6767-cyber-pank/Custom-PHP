...
// patched version by sk@mandarin-medien.de (2005)
...
// set picture attributes on change
	...
	{		
		...
		if (tinyMCE.getWindowArg('src') != '') {
			...
			var imgsrc = '<?php echo $tinyMCE_base_url.$imglib;?>'+ tinyMCE.getWindowArg('src').substr(tinyMCE.getWindowArg('src').lastIndexOf('/'),(tinyMCE.getWindowArg('src').length-tinyMCE.getWindowArg('src').lastIndexOf('/')));
			...
			parent.frames[frameID].location.href = imgsrc;
			updateStyle();					
		}			
...
<form name="libbrowser" method="post" action="ibrowser.php?request_uri=<?php echo $HTTP_GET_VARS['request_uri']?>" enctype="multipart/form-data" target="imglibrary">
  ...
  ?>
    	...
    	if ($d) 
    {
	  ...
	  sort($arr_tinyMCE_image_files);
		  ...
		  $size = getimagesize($tinyMCE_DOC_url.$imglib.$entry);
  ...
  ?>
	   ...
	   ?>
        	...
        	temprow['img_width'] = '<?php echo $size[0]; ?>';
        	temprow['img_height'] = '<?php echo $size[1]; ?>';
        	temprow['f_size'] = '<?php echo filesize_h($fsize,2); ?>'; 
